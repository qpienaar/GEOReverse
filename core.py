...
class CsgToCad:
    ...
    def build_universe(self, U: typing.Union[None, int] = None, depth: int = -1):
        ...
        The universe is built in its own coordinate system.
    ...
    def export_cad(self, output_filename: str = ""):
        ...
        i = 0
        size = len(self.buildCAD_list)
        for CAD in self.buildCAD_list:
            print(f"Processing CAD object {i} out of {size}, {100*(i/size)}% done.")
            CADobj.addObject(makeTree(CADdoc, CAD))
            i+=1
        print("Saving CAD object")
