...
class XmlInput:
    def __init__(self, name):
        ...
        self.__inputcards__, self.__lattices__ = get_cards(root)
        return
    ...
    def GetFilteredCells(self, Ustart, depth, matcel_list, settings):
        ...
        FilteredCells = {}
        for U in Ukeys:
            selected_cells = selectCells(self.Universes[U], matcel_list)
            for name, c in selected_cells.items():
                if c.geom is not None:
                    processSurfaces({name: c}, self.surfaces)
                    selected_cells[name] = CadCell(c, settings=settings)
                elif c.FILL is not None:          # fill cell, no geometry
                    ...
                    cad_c.surfaceList = []
                    if c.shape is None:
                        c.shape = Part.makeBox(3, 3, 214.2, FreeCAD.Vector(0, 0, 0))
                    cad_c.shape = c.shape
    ...
    def get_bounding_radius(self, fill_uid, universes, surfaces, visited=None):
        """
        Recursively estimate a bounding radius for a universe by scanning the
        numeric parameters of all surfaces reachable from it.

        The radius is computed as the maximum absolute value found across every
        numeric surface parameter in the universe and all universes it fills,
        giving a conservative upper bound on how far the geometry extends from
        the origin.  It is intended as a sizing heuristic for placeholder shapes
        (e.g. bounding boxes or spheres) that will subsequently be clipped by a
        container cell via ``interferencia()``, so over-estimation is safe and
        under-estimation is not.

        Parameters
        ----------
        fill_uid : int
            The universe ID to inspect.  Must be a key in ``universes``.
        universes : dict[int, dict[str, CellCard]]
            The full universe dictionary as built by ``GetLevelStructure``,
            mapping universe ID to a dict of cell name → cell card.
        surfaces : dict[int, Surface]
            The full surface dictionary as built by ``GetSurfaces``, mapping
            surface ID to a surface object.  Each surface object is expected to
            have a ``params`` attribute that is either a single numeric value, a
            FreeCAD Vector, or an iterable of such values; non-numeric entries
            are silently skipped.
        visited : set[int] or None, optional
            Tracks universe IDs already visited to prevent infinite recursion in
            cyclic or mutually-referencing fill structures.  Pass ``None``
            (the default) on the initial call; the function initialises the set
            internally.

        Returns
        -------
        float
            The maximum absolute numeric value found across all surface
            parameters reachable from ``fill_uid``, or ``0.0`` if no numeric
            parameters are found (e.g. the universe contains no surfaces, as is
            the case for a homogeneous implicit-complement cell such as the
            Reflector in a water assembly).
        """
        if visited is None:
            visited = set()
        if fill_uid in visited:
            return 0.0
        visited.add(fill_uid)

        # If fill_uid is a lattice, return pitch * dimension as the bounding radius
        if fill_uid in self.__lattices__:
            lattice = self.__lattices__[fill_uid]
            if len(lattice.dimensions) == 2: #2d lattice
                extents = [p * d for p, d in zip(lattice.pitch, lattice.dimensions)]
                extents.append(1e5) # size in z direction is unknown - let interferencia clip
            else:
                extents = [p * d for p, d in zip(lattice.pitch, lattice.dimensions)]
            return max(extents)

        if fill_uid not in universes:
            print("get_bounding_radius could not determine a bounding radius. Returning 0.0")
            return 0.0

        radius = 0.0
        for cell in universes[fill_uid].values():
            if hasattr(cell, 'surfaceList') and cell.surfaceList:
                for surf_id in cell.surfaceList:
                    surf = surfaces.get(surf_id)
                    if surf and surf.params:
                        for p in surf.params:
                            try:
                                radius = max(radius, abs(float(p)))
                            except TypeError:
                                pass
            if cell.FILL is not None:
                radius = max(radius, self.get_bounding_radius(cell.FILL, universes, surfaces, visited))
    ...
    def GetLevelStructure(self):
        ...
        for c in self.__inputcards__:
            ...
            if c.type == "cell":
                ...
                if c.FILL:
                    ...
                    containers_label.add(c.FILL) # using add seems to have worked
        ...
        for k in Universe_dict.keys():
            if k not in containers_label and k != root_universe:
                #raise RuntimeError(f"Universe {k} has no container cell.")
                print(f"Universe {k} has no container cell.")
...
def getSubUniverses(Ustart, Universes):
    ...
    for c in Universes[Ustart].values():
        if c.type == "cell":
            ...
            print(f"Unknown cell type, {c.type}, encountered when traversing tree structure")
...
def gq2cyl(x):
    ...
    # x.T*M*x + b.T*x + K = 0
    minWTol = 5.0e-2
    minRTol = 1.0e-2 # changed this from 1.0e-3
    # minRTol=3.e-1
    # lx = np.array(x)
    tp = ""
    M = np.array(
    ...
    if asw[0] < minWTol * asw[1]:
        ...
        if sw[1] == 0:
            print(f"Handling divide by zero... {sw[1]} being changed to 0.001")
            sw[1] = 0.001
